...
def gen(camera):
    ...
    while True:
        ...
        if recording:
            ...
            cv2.putText(frame,time.strftime("%I-%M-%S"), (1100,30), cv2.FONT_HERSHEY_SIMPLEX, 1, [255,255,255], 1)

        # Write the frame to the video
        out.write(frame)

        # Resize the frame so it can be streamed efficently
        frame = cv2.resize(frame, (680,420), interpolation = cv2.INTER_LINEAR)
